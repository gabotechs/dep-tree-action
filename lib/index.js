...
var require_package = __commonJS({
  "package.json"(exports, module2) {
    module2.exports = {
      ...
      scripts: {
        dev: "RUNNER_TEMP=/tmp ts-node src/index.ts",
...
var downloadURL = "https://github.com/gabotechs/dep-tree/releases/download";
function getAssetURL() {
  let ext = "tar.gz";
  let platform = import_os.default.platform().toString();
  switch (platform) {
    case "win32":
      platform = "windows";
      ext = "zip";
...
function install() {
  return __async(this, null, function* () {
    ...
    const depTreePath = import_path.default.join(extractedDir, `dep-tree`);
    core.info(`Installed dep-tree into ${depTreePath} in ${Date.now() - startedAt}ms`);
    return depTreePath;
  });
}
function asyncExec(cmd) {
  return new Promise((res) => (0, import_child_process.exec)(cmd, (error, stdout, stderr) => {
    res({ stdout, stderr, error });
  }));
}
function run() {
  return __async(this, null, function* () {
    const path2 = yield install();
    let configPath = core.getInput("config");
    if (configPath === "")
      configPath = ".dep-tree.yml";
    const {
      stdout,
      stderr,
      error
    } = yield asyncExec(`${path2} check --config ${configPath}`);
    if (stdout != null)
      core.info(stdout);
    if (stderr != null)
      core.info(stderr);
